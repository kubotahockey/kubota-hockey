document.addEventListener('DOMContentLoaded', () => {
    ...
    const playerChart = document.getElementById('playerChart'); // needed for chart render
    ...
    function displayPlayerComparisons(comps) {
        ...
        if (comps && comps.length > 0) {
            ...
            comps.forEach(comp => {
                ...
                let roundedGP = Math.round(comp.TOTAL_GP);
                let roundedGoals = Math.round(comp.TOTAL_G);
                let roundedAssists = Math.round(comp.TOTAL_A);
                let roundedPoints = Math.round(comp.TOTAL_PTS);
